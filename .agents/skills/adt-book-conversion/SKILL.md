...
## TTS highlighting must not deform text

Treat any visual movement caused by word highlighting as a blocking defect. Starting read-aloud, advancing the active word, pausing, stopping, changing language, or toggling easy-read must not change word spelling, punctuation, whitespace, line breaks, font metrics, baselines, element bounds, column alignment, or surrounding layout.

Audit representative and fragile text before, during, and after highlighting, including mixed-style runs, fixed-layout auto-fit text, headings, mathematics, tables, contents leaders, right-to-left or combining-mark text, and words containing apostrophes or hyphens. Compare DOM text and element geometry, not screenshots alone. If highlighting deforms a page, fix ADT's word-wrapper/tokenizer/runtime implementation and add focused regression coverage; never compensate by editing packaged HTML, shrinking fonts, widening boxes, or disabling highlighting without the user's explicit direction. Follow the complete TTS structure-preservation gate in the required reference.
