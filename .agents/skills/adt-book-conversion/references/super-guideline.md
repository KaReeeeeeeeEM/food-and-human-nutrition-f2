...
## Mandatory TTS word-highlighting structure-preservation gate

Word highlighting is an interactive rendering state, not permission to reconstruct or restyle the source text. A page fails acceptance if enabling read-aloud or moving the highlight changes the visual or semantic structure of any text.

For representative text on every page family—and every page after a systemic highlighting change—compare these states: initial render, read-aloud started, early/middle/final highlighted words, paused, stopped/cleared, language changed, and easy-read toggled when available.

1. **Text identity:** raw and normalized `textContent` must preserve every character, punctuation mark, separator, non-breaking space, and intentional line break. Apostrophes, hyphens, combining marks, non-Latin scripts, mathematics, and right-to-left text must remain intact. Tokenization may choose highlight boundaries but must never rewrite visible text.
2. **Inline structure:** wrappers must preserve the effective style and semantics of every original run, including colors, fonts, weights, sizes, decoration, language/direction, links, ruby markup, superscripts, subscripts, and accessible names. Do not flatten meaningful descendants into plain text.
3. **Whitespace integrity:** preserve exact separator rendering between styled runs. Do not collapse, duplicate, delete, or restyle ordinary/non-breaking spaces, indentation, or punctuation-adjacent spacing.
4. **Geometry invariance:** compare `getBoundingClientRect()` plus computed font and line metrics for containers and fragile descendants before wrapping, during multiple active words, and after teardown. Reject changes in position, size, baseline, line count, wrapping, column boundaries, table dimensions, or surrounding layout. Allow only a documented subpixel tolerance; visible movement always fails.
5. **Wrapper neutrality:** highlight wrappers must not introduce font size, line height, letter spacing, margins, padding, borders, vertical alignment, or display metrics that alter layout. Highlight color/background may change; text geometry may not. Nested wrappers must not create another baseline or make words bob as the active class moves.
6. **Fixed-layout auto-fit:** retain per-segment font metrics when rebuilding fixed-layout text and rerun the supported auto-fit mechanism when required. The wrapped result must match the pre-highlight fitted geometry and remain inside its positioned box.
7. **Idempotence and restoration:** wrapping an already wrapped element is a no-op. Stopping must restore original markup and geometry, not merely similar text. Repeated play/stop cycles, rapid skipping, and navigation must not accumulate spans, classes, inline styles, or stale original-HTML attributes.
8. **Dynamic rebuilds:** language/easy-read swaps, glossary highlighting, activity updates, and other DOM rewrites must coordinate with TTS wrappers. Rebind once to current DOM without stale indices, duplicate wrappers, lost styles, or deformation.
9. **Timing alignment:** visible word order and timestamps must use compatible Unicode-aware tokenization. When boundaries/counts differ, preserve the visible DOM and fall back safely rather than deleting characters or forcing mismatched fragments into wrappers.
10. **Regression evidence:** fix shared failures in ADT's tokenizer, word-wrapper, highlight CSS, or auto-fit lifecycle. Add focused tests for text identity, styled runs, idempotence, restoration, and geometry-relevant behavior. Run runtime tests and inspect the final packaged output in a real browser; unit tests alone do not satisfy acceptance.

Test ordinary prose, punctuation/quotes, apostrophes/hyphens, consecutive styled spans, large headings, fixed-layout auto-fit paragraphs, tables/contents rows, inline mathematics, links, combining marks/non-Latin text present in the book, translated text, and repeated start/stop cycles.

Do not fix this defect by modifying packaged HTML, adding global `white-space` rules, shrinking typography, enlarging source boxes, or turning off word highlighting. Correct the durable ADT runtime unless the user explicitly chooses a fallback mode.
